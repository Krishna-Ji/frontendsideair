...
const Photosuploader = ({ addedPhotos, setAddedPhotos, photoLink, setPhotoLink }) => {
  ...
  async function addPhotoByLink(ev) {
    ...
    try {
      const response = await axios.post('https://back-airhoster-1.onrender.com/uploadby', { link: photoLink });
  ...
  function uploadPhoto(ev) {
    ...
    axios.post('https://back-airhoster-1.onrender.com/uploads', data, {
      headers: {
        ...
        {addedPhotos && addedPhotos.map((link) => {
          ...
          return (
            <div className='h-32 flex relative' key={link}>
              <img key={link} src={'https://back-airhoster-1.onrender.com/uploads/' + link} className='w-full rounded-2xl' />
